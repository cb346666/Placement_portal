<?php
	...
	if(empty($info['errors']) && $row)
	{
		...
		db_query("update user set firstname = :firstname,lastname = :lastname,gender = :gender,date = :date,email = :email $image_query $password_query where id = :id limit 1",$arr);
		...
		$row = db_query("select * from user where id = :id limit 1",['id'=>$row['id']]);
		if($row)
		{
			$row = $row[0];
